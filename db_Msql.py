...
def Initialize_Msql(app):
    ...
    # This is username with access to the database
    app.config['MYSQL_USER'] = 'dev'
    # This is the password of the username
    app.config['MYSQL_PASSWORD'] = 'dinero0123'
...
def Update_Password(Email, New_Password, cursor, mYSQL):
    ...
    if (True == cursor.execute('SELECT * FROM Employees WHERE Email = %s', (Email,))):
        # Write command to mysql this command writes to the Employees.table
        cursor.execute(
            'UPDATE Employees set Password = %s WHERE Email = %s', (New_Password, Email,))
    ...
    if (True == cursor.execute('SELECT * FROM Company WHERE Email = %s', (Email,))):
        ...
        cursor.execute(
            'UPDATE Company set Password = %s WHERE Email = %s', (New_Password, Email,))
        # Commit the execute command
        mYSQL.connection.commit()
...
def find_lot_by_company(company_id, cursor):
    cursor.execute(
        'SELECT * FROM Products WHERE company_id = %s', (company_id, ))
...
def add_product(Lot_id, Name, Brand, product_id, company_id, EPA, PHI, REI, Temp_type, Temp, cursor, mYSQL):
    ...
    cursor.execute("INSERT INTO Platform_abacrop.Products (Lot_id, company_id, product_id, Name, Brand, EPA, PHI, REI, Temperature, TemperatureTypeId) VALUES (%s, %s, %s, %s,     %s,  %s,  %s,  %s, %s)",
                   (Lot_id, company_id, product_id, Name, Brand, EPA, PHI, REI, Temp_type, Temp))
    mYSQL.connection.commit()
######### PRODUCTS SECTION ############
